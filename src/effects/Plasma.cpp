...
namespace MoleDemo
{
	...
	void Plasma::Load() {
		...
		for (unsigned int j = 0; j < screen->h * 2; ++j) {
			for (unsigned int i = 0; i < screen->w * 2; ++i) {
				plasma1.push_back((channel)(64 + 63 * (std::sin((double)std::hypot(screen->h - j, screen->w - i) / 16))));
				plasma2.push_back((unsigned char)(64 + 63 * std::sin((float)i / (37 + 15 * std::cos((float)j / 74)))
	...
	void Plasma::buildPalette(uint16_t time) {
		...
		{
			palette[i] = {
				(unsigned char)(128 + 127 * std::cos(i * PI / 128 + (double)time / 740)),
