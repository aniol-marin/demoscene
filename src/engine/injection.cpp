...
/*
   ...
   std::map<id_capacity, std::unique_ptr<Interface>> instances;
   const std::function< std::unique_ptr<Interface>()> functor;
   public:
   TestSharingFactory(std::function< std::unique_ptr<Interface>()> functor, id_capacity id = 0) :
   instances{},
   functor{ functor },
   Factory{} {}
   ...
   Interface* GetSharedObject(id_capacity id = 0) {
   ...
   void PushInstance(std::unique_ptr<Interface>&& instance, id_capacity id = 0) {
   ...
   const std::function<std::unique_ptr<Interface>()> functor;
   public:
   TestInstanceFactory(std::function<std::unique_ptr<Interface>()> functor, id_capacity id = 0) :
   functor{ functor },
   Factory{} {}
   ~TestInstanceFactory() override {
   }

   Interface* GetObject(id_capacity _ = 0) {
...
*/
/*
   std::map<id_capacity, std::unique_ptr<Factory>> newInstanceFactories; // UniqueFactories
   std::map<id_capacity, std::unique_ptr<Factory>> sharedInstancesFactories; // SharingFactories
*/
/*
...
// Returns a shared pointer to a unique instances
template<typename T>
std::shared_ptr<T> InjectSharedInstance(id_capacity id = 0) {
...
void PushInstance(std::unique_ptr<IInstance>&& instance, id_capacity id = 0) {
